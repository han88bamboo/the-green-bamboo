...
def get_rank_by_user_id(user_id):
    ...
    Returns:
        str: The rank of the user.
    """
    # Retrieve the user's proof points from the database using connection pooling
...
def check_max_proof_points(user_id):
    ...
    # Retrieve the max proof points from the database using connection pooling
    with db_manager.get_cursor() as cursor:
...
def check_user_can_create_club(user_id):
    ...
    # Retrieve the minimum proof points from the database using connection pooling
    with db_manager.get_cursor() as cursor:
        ...
        if current_points['currentPoints'] >= min_points:
            ...
            cursor.execute('SELECT COUNT(*) FROM "clubs" WHERE "createdByID" = %s AND "createdByType" = %s', (user_id, 'user',))
            club_count = cursor.fetchone()
...
def check_user_can_create_event(user_id):
    ...
    with db_manager.get_cursor() as cursor:
        ...
        if current_points['currentPoints'] >= min_points_event:
            ...
            cursor.execute('SELECT COUNT(*) FROM events WHERE "eventOwnerID" = %s AND "eventOwnerType" = %s AND "createdDate" >= date_trunc(\'month\', CURRENT_DATE)', (user_id, 'user',))
