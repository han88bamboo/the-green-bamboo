...
def updateBookmark():
    ...
    try:
        ...
        cursor.execute('SELECT "id", "listName" FROM "userProducerLists" WHERE "userId" = %s', (userID,))
        existing_producer_lists = {row['listName']: row['id'] for row in cursor.fetchall()}
        ...
        producer_bookmark_list_names = set(bookmark.get('producer_lists', {}).keys())
        existing_producer_list_names = set(existing_producer_lists.keys())

        producer_lists_to_delete = existing_producer_list_names - producer_bookmark_list_names
        for listName in producer_lists_to_delete:
            cursor.execute('DELETE FROM "userProducerLists" WHERE "userId" = %s AND "listName" = %s', (userID, listName))
        ...
        for listName, listData in bookmark.get('producer_lists', {}).items():
            listItems = listData["listItems"]

            if listName in existing_producer_lists:
                list_id = existing_producer_lists[listName]
