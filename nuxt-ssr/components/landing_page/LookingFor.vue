<template>
  ...
  <section class="category-ribbon-section pt-0">
    <div class="category-ribbon-bar">
      <div class="container">
        <h3 class="mb-0 pt-2 fw-bold text-center" style="color:white;">I'm Looking For</h3>
        ...
        <div class="category-ribbon-nav">
          ...
          <div v-for="category in categories" :key="category.id" class="category-item"
            ...
            <NuxtLink :to="`/browse/${category.params.browseDrinkType}`" class="category-link"
              @click="toggleMobileCategoryMenu(category, $event)">
              {{ category.name }}
            </NuxtLink>
            ...
            <div class="mega-menu" :ref="el => menuRefs[category.id] = el" :class="{
              ...
              <div class="mega-menu-content">
                <!-- Main Category Link -->
                <NuxtLink :to="`/browse/${category.params.browseDrinkType}`" class="subcategory-link main-category">
                  {{ category.name }}
                </NuxtLink>
                <!-- Subcategory Links -->
                <NuxtLink v-for="sub in category.subcategories" :key="sub.name"
                  :to="`/browse/${sub.params.browseDrinkType}/${sub.params.browseTypeCategory}`" class="subcategory-link">
                  {{ sub.name }}
                </NuxtLink>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    toggleMobileCategoryMenu(category, event) {
