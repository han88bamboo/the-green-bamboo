  ...
  methods: {
    ...
    onSubmitImage() {
      ...
      if (this.uploadedImage || this.imageLink.trim()) {
            ...
            if (logo || labels.length > 0 || detectedText) {
              ...
              this.$router.push({
                ...
                query: {
                  ...
                  detectedText: encodeURIComponent(
                    JSON.stringify(detectedText)
                  ),
                },
              });
            } else {
        ...
        this.triggerPopup(
          "No image detected, please upload an image or enter a link before submitting!"
        );
      }
    },
    ...
    async reverseImageSearch(image) {
      try {
        ...
        let detectedLogo =
          visionData.responses[0]?.logoAnnotations?.[0]?.description || null;
        ...
        let detectedText =
          visionData.responses[0]?.textAnnotations?.map(
            (detectedText) => detectedText.description
          ) || [];
