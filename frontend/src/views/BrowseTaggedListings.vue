                ...
                <div class="col-md-8 col-12">
                    ...
                    <div class="row d-flex justify-content-center mb-2">
                        ...
                        <div class="col-lg-2 col-md-3 col-4 mb-2 dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <li class="px-3 py-3" style="min-width: 250px;">
                                    <div class="mb-3">
                                        <label class="form-label small mb-2 text-center d-block">Rating Range: {{ minRatingDisplay }} - {{ maxRatingDisplay }}</label>
                                        <div class="range-slider-container position-relative">
                                            <!-- Track -->
                                            <div class="range-track"></div>
                                            <!-- Progress between handles -->
                                    ...
                                    <div class="d-flex gap-2">
                                        <button class="btn btn-outline-secondary btn-sm flex-fill" @click="resetRatingRange">Reset</button>
                                        <button class="btn btn-primary btn-sm flex-fill" @click="applyRatingRange">Apply</button>
                                    </div>
                                </li>
                            </ul>
                        </div>
    ...
    export default {
        ...
        computed: {
            ...
            progressStyle() {
                const min = 1;
                const max = 10;
                const leftPercent = ((this.minRatingValue - min) / (max - min)) * 100;
                const rightPercent = 100 - ((this.maxRatingValue - min) / (max - min)) * 100;
                return {
                    left: leftPercent + '%',
                    right: rightPercent + '%'
                };
        ...
        methods: {
            ...
            updateRatingRange() {
                ...
                if (parseInt(this.minRatingValue) > parseInt(this.maxRatingValue)) {
                    this.minRatingValue = this.maxRatingValue;
                }
            },

            applyRatingRange() {
                // Only apply if not at default full range
                if (this.minRatingValue > 1 || this.maxRatingValue < 10) {
                    this.browseFilters.minRating = this.minRatingValue.toString();
                    this.browseFilters.maxRating = this.maxRatingValue.toString();
                } else {
...
.range-input:focus::-webkit-slider-thumb {
    box-shadow: 0 0 0 2px rgba(131, 169, 232, 0.3);
}
