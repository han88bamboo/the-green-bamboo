...
<template>
    ...
    <div class="container pt-5 mobile-pt-3" v-if="dataLoaded" style="position: relative;">
        ...
        <div class="row">
            ...
            <div class="col-lg-4 col-md-12 col-sm-12">
                ...
                <div class="row">
                    ...
                    <div class="col-4">
                        <img :src="selectedImage || 'data:image/jpeg;base64,' + (photo || defaultProfilePhoto)" alt=""
                ...
                <div class="row pt-3 mobile-view-hide">
                    <router-link :to="'/profile/producer/' + user_id"
                        class="btn primary-btn-outline-thick rounded-0 default-clickable-text">
                        Return to profile
                    </router-link>
                    <router-link :to="'/Producers/ProducersDashboard/' + user_id"
                        class="btn primary-btn-outline-thick rounded-0 default-clickable-text mt-3">
                        Return to analytics
                    </router-link>
                </div>
                ...
                <div>
                    ...
                    <div class="row mx-2">
                        ...
                        <div class="col-4 text-end">
                            <button type="button" class="btn primary-btn-outline-not-round"
                                style="font-weight:bold; width: 200px;">Edit Details</button>
                        </div>
                    </div>
                </div>
                <!-- payment method -->
                <div>
                    ...
                    <div class="row mx-2 mb-3">
                        <div v-if="subscription">
                            <!-- stripe -->
                            <img v-if="paymentMethod?.brand == 'visa'" src="../../Images/Others/visa.png" alt=""
                                style="height: 40px;" class="me-3">
                            <img v-else-if="paymentMethod?.brand == 'mastercard'"
                                src="../../Images/Others/mastercard.png" alt="" style="height: 40px;" class="me-3">
                            <img v-else src="../../Images/Others/credit-card.png" alt="" style="height: 30px;"
                                class="me-3">
                            •••• •••• •••• {{ paymentMethod?.last4 }}
                        </div>
                        <div v-else>
                ...
                <div>
                    ...
                    <div v-if="subscription" class="row mx-2">
                        ...
                        <div v-if="activeSubscription" class="col-6 text-end">
                            <div class="fw-bold fs-3">${{ subscriptionDetails?.price }}
                                p/{{ subscriptionDetails?.interval }}</div>
                            <div class="fst-italic">Next billing cycle: {{
                                formatDate(subscriptionDetails?.next_billing_date) }}</div>
                            <div class="fst-italic">Amount due: ${{ (this.upcomingInvoice.amount_due / 100).toFixed(2)
                                }}</div>
                            <button type="button" class="btn primary-btn-outline-not-round mt-2"
                                style="font-weight:bold; width: 200px;" data-bs-toggle="modal"
                                data-bs-target="#editSubscriptionModal">Edit Subscription</button>
                        </div>

                        <div v-else class="col-6 text-end">
                            <div class="fw-bold fs-3" style="color: red;">PAUSED</div>
                            <div v-if="activeSubscription == false" class="fst-italic" style="color: red;">Your
                                subscription is available until {{ cancelledDate }}.</div>
                            <div class="fst-italic" style="color: red;">Resume subscription to access premium features.
                            </div>
                            <button type="button" class="btn primary-btn-not-round mt-2"
                                style="font-weight:bold; width: 200px; background-color: #71966B; border: 4px solid #71966B;"
                                @click="resumeSubscription">Resume Subscription</button>
                        </div>
                    </div>
                    <div v-else class="row mx-2">
                        ...
                        <div class="col-6 text-end">
                            <button type="button" class="btn primary-btn-not-round mt-2"
                                style="font-weight:bold; width: 200px; background-color: #71966B; border: 4px solid #71966B;"
                                @click='reSubscribe'>Subscribe</button>
                        </div>
        ...
        <div> <!-- modals -->
            ...
            <div class="modal fade" id="editSubscriptionModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-start">
                            ...
                            <div class="row justify-content-between">
                                <div class="col-6">
                                    <button class="btn rounded p-3 text-start mb-3 w-100" @click="toggleMonthlyPricing"
                                        :style="{
                                            backgroundColor: selectedMonthlyPricing ? '#DD9E54' : 'white',
                                ...
                                <div class="col-6">
                                    <button class="btn rounded p-3 text-start mb-3 w-100" @click="toggleYearlyPricing"
                                        :style="{
                                            backgroundColor: selectedYearlyPricing ? '#DD9E54' : 'white',
                            ...
                            <div v-if="selectedMonthlyPricing && subscriptionDetails?.interval != 'month'"
                                class="alert alert-danger" role="alert">You are changing your subscription plan to <span
                                    class="fw-bold">monthly</span>. </div>
                            <div v-else-if="selectedYearlyPricing && subscriptionDetails?.interval != 'year'"
                                class="alert alert-success" role="alert">You are changing your subscription plan to
                                <span class="fw-bold">yearly</span>. </div>
                            <div v-else class="alert alert-light" role="alert">No changes selected.</div>
                            <hr>
                            <div>If you wish to pause your subscription, please click <span data-bs-toggle="modal"
                                    data-bs-target="#cancelSubscriptionModal"
                                    style="text-decoration: underline; cursor: pointer;">here</span>.</div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                            <button v-if="selectedMonthlyPricing && subscriptionDetails?.interval != 'month'"
                                type="button" class="btn btn-primary" @click="editSubscription('month')"
                                data-bs-dismiss="modal">Confirm</button>
                            <button v-else-if="selectedYearlyPricing && subscriptionDetails?.interval != 'year'"
                                type="button" class="btn btn-primary" @click="editSubscription('year')"
                                data-bs-dismiss="modal">Confirm</button>
            ...
            <div class="modal fade" id="cancelSubscriptionModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <div style="display: inline" class="text-center mt-4 mb-4">
                            <button type="button" class="btn btn-secondary me-3" data-bs-toggle="modal"
                                data-bs-target="#editSubscriptionModal">Keep Subscription</button>
                            <button type="button" class="btn btn-danger" data-bs-dismiss="modal"
                                @click="cancelSubscription">Pause Subscription</button>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async loadData() {
            ...
            // business
            try {
                const response = await this.$axios.get(`${process.env.VUE_APP_API_URL}/getData/get${this.userType.charAt(0).toUpperCase()}${this.userType.slice(1)}/${this.user_id}`);
        ...
        async getPaymentMethodIdFromCustomerId(customerId) {
            try {
                const response = await this.$axios.get(`${process.env.VUE_APP_API_URL}/getData/get-payment-method-id/${customerId}`);
