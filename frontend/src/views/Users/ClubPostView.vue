            ...
            <div class="container mt-3 text-start">
                ...
                <div v-if="isMember" class="row text-start">
                    <div class="col-12 d-flex gap-4">
                        ...
                        <p v-if="(post.likedMembers).includes(memberID)" data-bs-toggle="tooltip" data-bs-placement="top" title="Unlike" class="cursor-pointer">
                ...
                <div v-if="isMember" class="row mt-3">
                    <div class="col-12">
                        <div class="input-group">
                            <input type="text" class="form-control" placeholder="Write a comment..." aria-label="Write a comment..." aria-describedby="button-addon2" v-model="newComment">
                            <button class="btn primary-btn" type="button" id="button-addon2" @click="addComment">Comment</button>
                        </div>
                    </div>
                </div>
                        ...
                        <div v-for="comment in comments" :key="comment.id" class="row mt-3">
                            ...
                            <div class="col-11">
                                ...
                                <div class="row text-start d-flex align-items-center ps-0">
                                    <div class="col-md-3">
                                        <router-link :to="profileURL(comment.commenterInfo.id, comment.commenterInfo.userType)" class="text-black">
                                            <p v-if="comment.commenterInfo.userType == 'user'" class="fw-bold">{{ comment.commenterInfo.displayName }}</p>
                                            <p v-else-if="comment.commenterInfo.userType == 'producer'" class="fw-bold">{{ comment.commenterInfo.producerName }}</p>
                                            <p v-else class="fw-bold">{{ comment.commenterInfo.venueName }}</p>
                                        </router-link>
                                    </div>
                                    <div class="col-md-5">
                                    ...
                                    <div class="col-md-4 text-end">
                                        ...
                                        <button v-if="comment.commenterID == memberID || isAdmin" class="btn primary-btn-green btn-sm me-3" data-bs-toggle="modal" data-bs-target="#editCommentModal" @click="selectedComment = deepCopy(comment)">Edit</button>
                                        <button v-if="comment.commenterID == memberID || isAdmin" class="btn primary-btn-red btn-sm" data-bs-toggle="modal" data-bs-target="#deleteCommentModal" @click="selectedCommentDelete = deepCopy(comment)">Delete</button>
                                    </div>
                                </div>
                                ...
                                <div class="row d-flex align-items-center">
                                    <div class="col-12 d-flex gap-4">
                                        <!-- Red thumbs up with red fill if user already liked the post -->
                                        <p v-if="(comment.likedMembers).includes(memberID)" data-bs-toggle="tooltip" data-bs-placement="top" title="Unlike" class="cursor-pointer">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="red" class="bi bi-hand-thumbs-up-fill" viewBox="0 0 16 16"
                                                style="cursor: pointer;" @click="likeComment(comment.id)">
                                                <path d="M6.956 1.745C7.021.81 7.908.087 8.864.325l.261.066c.463.116.874.456 1.012.965.22.816.533 2.511.062 4.51a10 10 0 0 1 .443-.051c.713-.065 1.669-.072 2.516.21.518.173.994.681 1.2 1.273.184.532.16 1.162-.234 1.733q.086.18.138.363c.077.27.113.567.113.856s-.036.586-.113.856c-.039.135-.09.273-.16.404.169.387.107.819-.003 1.148a3.2 3.2 0 0 1-.488.901c.054.152.076.312.076.465 0 .305-.089.625-.253.912C13.1 15.522 12.437 16 11.5 16H8c-.605 0-1.07-.081-1.466-.218a4.8 4.8 0 0 1-.97-.484l-.048-.03c-.504-.307-.999-.609-2.068-.722C2.682 14.464 2 13.846 2 13V9c0-.85.685-1.432 1.357-1.615.849-.232 1.574-.787 2.132-1.41.56-.627.914-1.28 1.039-1.639.199-.575.356-1.539.428-2.59z"/>
                                            </svg>
                                        </p>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <textarea v-if="selectedComment" class="form-control" v-model="selectedComment.commentContent"></textarea>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" :disabled="isDisabled">Close</button>
                                            <button type="button" class="btn btn-primary" @click="editComment()" data-bs-dismiss="modal" :disabled="isDisabled">Save changes</button>
                                        </div>  
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="modal fade" id="deleteCommentModal" tabindex="-1" aria-labelledby="deleteCommentModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" :disabled="isDisabled">Close</button>
                                            <button type="button" class="btn btn-danger" @click="deleteComment()" data-bs-dismiss="modal" :disabled="isDisabled">Delete</button>
                                        </div>
                                    </div>  
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        ...
        async addComment() {
            try {
                // Comment on the post
                const commentData = await this.$axios.post(`${process.env.VUE_APP_API_URL}/club/addComment`, {
                    postID: this.postID,
        ...
        async likeComment(commentID) {
            try {
                // Like the comment
                const likeData = await this.$axios.put(`${process.env.VUE_APP_API_URL}/club/likeUnlikeComment`, {
                    commentID: commentID,
                    memberID: this.memberID,
                    postID: this.postID
                });
        ...
        async editComment() {
            try {
                ...
                // Edit the comment
                const editData = await this.$axios.put(`${process.env.VUE_APP_API_URL}/club/editComment`, {
                ...
                if (editData.status == 200) {
                    // Find the comment in the comments array
                    const comment = this.comments.find(comment => comment.id == this.selectedComment.id);
                ...
                const toast = useToast();
                toast.error("An error occurred while editing the comment. Please try again later.");
            }
        ...
        async deleteComment() {
            try {
                // Delete the comment
                const deleteData = await this.$axios.delete(`${process.env.VUE_APP_API_URL}/club/removeComment`, {
                    data: {
                ...
                if (deleteData.status == 200) {
                    // Find the comment in the comments array
                    const index = this.comments.findIndex(comment => comment.id == this.selectedCommentDelete.id);
                    if (index > -1) {
                ...
                const toast = useToast();
                toast.error("An error occurred while deleting the comment. Please try again later.");
            }
        }
    },
    mounted() {
