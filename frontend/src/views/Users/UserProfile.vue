                  ...
                  <div class="modal-body text-center">
                    ...
                    <div class="edit-drink-choice">
                      ...
                      <div class="row pt-5">
                        ...
                        <div class="col-8 text-start">
                          <div
                            v-for="(tag, index) in flavourTags"
              ...
              <div v-else class="container text-center mb-3">
                ...
                <div class="row">
                  <div
                    ...
                    v-for="drinkTypeDetails in matchedDrinkTypes"
                    :key="drinkTypeDetails.id"
                  >
                ...
                <div class="row">
                  ...
                  >
                    ...
                    <!-- <img :src="'data:image/png;base64,'+ (getBadgeInfo(badge).badgePhoto)" 
                                            alt="" class="rounded-circle-white-bg border border-dark badge-img"> -->
                    <img
                      :src="getBadgeInfo(badge)?.badgePhoto"
                      alt=""
                      class="rounded-circle-white-bg border border-dark badge-img"
                    />
                    <!-- badge description -->
            ...
            <div class="tab-content container mt-2 mobile-px-0">
              ...
              <div v-if="activeTab == 'reviews'" id="reviews">
                ...
                <div v-if="Object.keys(recentReviews).length > 0">
                  <div
                    v-for="(review, index) in recentReviews.slice(0, 5)"
                    :key="index"
                  >
              ...
              <div v-if="activeTab == 'lists'" id="lists">
                ...
                >
                  <div class="col-3 mobile-col-4 mobile-pe-0">
                    ...
                    <img
                      :src="
                        getListingFromID(bookmarkList.listItems[0])?.photo ||
                        defaultDrinkImage
                      "
                      alt=""
...
export default {
  ...
  data() {
    return {
      ...
      // for points
      pointSystem: {
        // CHANGE THIS! if there is a change in the point system (just change this.pointsDefault to a numerical value for the corresponding criteria)
  ...
  computed: {
    formattedModTypes() {
      ...
      if (this.displayUser.modType.length === 0) {
        ...
        return (
          "This user is currently a moderator for <b>" +
          this.displayUser.modType.join(", ") +
          "</b>!"
        );
      }
    },
  ...
  methods: {
    ...
    async loadData(url) {
      ...
      try {
        ...
        this.recentReviews = this.reversedReviews.filter(
          (review) =>
            review.userID === parseInt(this.displayUserID) &&
            review.reviewType === "Listing"
        );
      } catch (error) {
        console.error(error);
        this.dataLoaded = null;
    ...
    async saveChangesDetails() {
      ...
      try {
        ...
        const response = await this.$axios.post(
          `http://127.0.0.1:5000/editProfile/editDetails`,
          {
            userID: this.userID,
    ...
    searchResult() {
      this.drinkSearchResults = this.listings
        .filter((listing) =>
          listing.listingName
            .toLowerCase()
            .includes(this.drinkSearch.toLowerCase())
        )
    ...
    getUserFavourite() {
      ...
      const favouriteListingIds = favouriteListingReviews.map(
    ...
    getRecentActivity() {
      ...
      // bookmarks
      let allUserBookmarks = Object.values(this.userBookmarks)
        .map((item) => item.listItems)
        .flat();
      let allUserBookmarksDict = allUserBookmarks.map((listingID) => ({
        listingID,
      }));
      ...
      this.recentActivity = userActivity
        ...
        .map((item) => {
          item = this.listings.find(
            (listing) => listing.id === parseInt(item.listingID)
          );
          return item;
        })
        .slice(0, 5);
    ...
    getAverageReview(listingID) {
      ...
      const reviews = this.reviews.filter(
        (review) => review.reviewTarget === listingID
      );
      if (reviews.length > 0) {
    ...
    getAllListingsReviewed() {
      this.allListingsReviewedByUser = this.recentReviews.map((review) =>
        this.listings.find((listing) => listing.id === review.reviewTarget)
      );
    },
    ...
    getAllCategoriesReviewed() {
      for (let listing of this.allListingsReviewedByUser) {
        ...
        this.allCategoriesReviewedByUser[listing.drinkType] =
          (this.allCategoriesReviewedByUser[listing.drinkType] || 0) + 1;
    ...
    getTopCategoriesReviewed() {
      this.topCategoriesReviewed = Object.keys(
        this.allCategoriesReviewedByUser
      ).reduce((acc, category) => {
        ...
        for (let subcategory in this.allSubCategoriesReviewedByUser[category]) {
          let count =
            this.allSubCategoriesReviewedByUser[category][subcategory];
        ...
        if (
          this.allCategoriesReviewedByUser[category] >= this.badgeLevels.master
        ) {
        ...
        // --> MIDDLE TIER: LOVER (>= 10 reviews)
          ...
          this.allCategoriesReviewedByUser[category] >= this.badgeLevels.lover
        ) {
          acc[category] = this.allCategoriesReviewedByUser[category];
        ...
        // --> LOWEST TIER: NOVICE (>= 3 reviews)
          ...
          this.allCategoriesReviewedByUser[category] >= this.badgeLevels.novice
        ) {
          acc[category] = this.allCategoriesReviewedByUser[category];
    ...
    getMatchedDrinkType() {
      this.matchedDrinkTypes = Object.keys(this.topCategoriesReviewed).map(
        (category) =>
          this.drinkTypes.find((drinkType) => drinkType.drinkType === category)
      );
    },
    ...
    checkEnoughLocationTagged() {
      if (
        this.pointSystem.tagLocation[0] >= this.otherBadgesLimit.tagLocation
      ) {
      ...
      let totalUpvotes = this.recentReviews.reduce((count, review) => {
        if (
          review.userVotes.upvotes.some(
            (vote) => vote.userID === this.displayUserID
          )
        ) {
          count += 1;
        }
    ...
    getTotalFriendsTagged() {
      ...
      this.pointSystem.tagFriend[0] = this.recentReviews.reduce(
        (sum, review) => sum + review.taggedUsers.length,
        0
      );
    },
    ...
    getTotalLocationsTagged() {
      ...
      this.pointSystem.tagLocation[0] = this.recentReviews.reduce(
        (sum, review) => {
          if (review.location) {
            return sum + 1;
          }
    ...
    getAskedProducerQuestions() {
      this.pointSystem.askProducer[0] = this.producers.reduce(
        (totalQuestions, producer) => {
          return (
            totalQuestions +
            producer.questionsAnswers.filter(
              (qa) => qa.userID === this.displayUserID
            ).length
          );
        },
        0
    ...
    getAskedVenueQuestions() {
      this.pointSystem.askVenue[0] = this.venues.reduce(
        (totalQuestions, venue) => {
          return (
            totalQuestions +
            venue.questionsAnswers.filter(
              (qa) => qa.userID === this.displayUserID
            ).length
          );
        },
    ...
    calculateTotalPoints() {
      ...
      this.totalPoints = Object.values(this.pointSystem).reduce(
