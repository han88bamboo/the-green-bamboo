    ...
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        ...
        <div class="modal-body modal-body-scrollable">
          ...
          <div class="border rounded p-3 mb-4" style="background-color: #fafafa;">
            ...
            <div class="form-group mb-3">
              ...
              <div v-if="newItem.selectedProducer && newItem.selectedProducer.id" class="mt-2 p-2 bg-light border rounded">
            ...
            <div class="form-group mb-3">
              <p class="text-start mb-1">Drink Name<span class="text-danger">*</span>
                <span class="text-muted" style="font-size: 14px;">Just begin typing, then select from the drop-down suggestions.</span>
                <span v-if="newItem.selectedProducer && newItem.selectedProducer.id" class="text-info fw-bold" style="font-size: 14px;">
                  - Filtered by {{ newItem.selectedProducer.producerName }}
                </span>
              </p>
              <input type="text" class="form-control" v-model="newItem.searchQuery"
                @input="debouncedSearchListings"
              ...
              <ul class="list-group" v-if="newItem.searchResults && newItem.searchResults.length > 0 && newItem.searchQuery">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-danger" data-bs-dismiss="modal" @click="clearAddedItems">Cancel</button>
...
import { VARIANT_DRNK_TYP } from '@/utils/constants';
...
export default {
  ...
  emits: ['menu-items-added'],
  data() {
    return {
      newItem: this.getInitialNewItemState(),
      addedMenuItems: [],
      servingTypes: [],
      VARIANT_DRNK_TYP: VARIANT_DRNK_TYP,
    };
  },
  computed: {
    isNewItemValid() {
  ...
  methods: {
    ...
    async submitAddedItems() {
      if (this.addedMenuItems.length === 0) return;
      if (!this.targetSection || !this.targetSection.sectionName) {
          alert("Error: No target menu section provided.");
          return;
      }

      const promises = this.addedMenuItems.map(item => {
        const payload = {
          venueID: this.venueId,
          listingID: item.newMenuItemID,
          itemVintage: item.newMenuItemVintage,
          itemPrice: item.newMenuItemPrice || 0,
          servingType: item.newMenuItemServingType,
          sectionName: this.targetSection.sectionName,
        };
        return axios.post(`${process.env.VUE_APP_API_URL}/editVenueProfile/addListingToMenu`, payload);
      });

      try {
        await Promise.all(promises);
        this.$emit('menu-items-added');
        this.clearAddedItems();
      } catch (error) {
        console.error('Error adding menu items:', error);
        alert("An error occurred while adding items to the menu. Please try again.");
      }
    }
  },
  created() {
